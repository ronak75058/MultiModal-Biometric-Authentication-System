...
def get_attempt_path_from_staging(image_type):
    ...
    while True:
        ...
        print(f"  -> Name it: '{base_filename}.png' OR '{base_filename}.jpg' OR '{base_filename}.jpeg'")
        print("=" * 50)
        ...
        path_jpeg = os.path.join(staging_folder, f"{base_filename}.jpeg")
...
def main():
    ...
    while True:
        print("\n" + "-" * 50)
        choice = input(
            "Do you want to (r)egister, (a)uthenticate, or (d)elete a user? (q to quit): "
        ).strip().lower()
        ...
        # ---------------- DELETE ----------------
            ...
            confirm = input(
                f"Are you sure you want to permanently delete all data for '{username}'? (yes/no): "
            ).strip().lower()
            if confirm == 'yes':
        ...
        # ---------------- AUTHENTICATE ----------------
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT face_data_path, fingerprint_data_path, signature_data_path FROM users WHERE username=?",
                (username,)
            )
