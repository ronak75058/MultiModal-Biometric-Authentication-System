    ...
    while time.time() - start_time < 10:
        ...
        for face_encoding, face_location in zip(face_encodings, face_locations):
            ...
            if average_distance < TOLERANCE:
                text = "Authenticated!"
                cv2.putText(frame, text, (left, bottom + 25), cv2.FONT_HERSHEY_DUPLEX, 0.8, (0, 255, 0), 2)
                authentication_successful = True
            else:
                text = f"Dist: {average_distance:.2f}"
                cv2.putText(frame, text, (left, bottom + 25), cv2.FONT_HERSHEY_DUPLEX, 0.8, (255, 255, 255), 1)
